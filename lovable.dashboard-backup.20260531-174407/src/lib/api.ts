...
export const getChatMessages = (sessionId: string) =>
  fetchApi<any>(`/chat/history/${encodeURIComponent(sessionId)}`);
...
// Zeroclaw model-selection surface. The server remains authoritative:
// Zeroclaw is the wg-xray runtime surface; providers/models are nested beneath it.
export const getLlmStatus = () => fetchApi<any>("/llm/status");
export const getLlmProviders = () => fetchApi<any>("/llm/providers");
export const getLlmModels = (provider?: string) =>
  fetchApi<any>(provider ? `/llm/models/${encodeURIComponent(provider)}` : "/llm/models");
export const switchLlmProvider = (provider: string) =>
  fetchApi<any>("/llm/provider", {
    method: "POST",
    body: JSON.stringify({ provider }),
  });
export const switchLlmModel = (model: string) =>
  fetchApi<any>("/llm/model", {
    method: "POST",
