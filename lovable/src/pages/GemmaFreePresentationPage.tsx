...
import { TrendingUp, Search, Layers, Zap } from "lucide-react";
import { api } from "@/lib/api";

interface FieldInfo { name: string; control_type?: string; example?: unknown; }
...
const PROMPT = (pluginId: string, fields: FieldInfo[], description: string) =>
  `You are an intelligent system analyst looking at a plugin called "${pluginId}" in a live infrastructure dashboard.

What it does: ${description}
Fields available: ${fields.map(f => `${f.name} (${f.control_type ?? "unknown"})`).join(", ")}

Imagine you're presenting this data to a team. What single visualization — chart, graph, timeline, heatmap, or anything else — would make someone stop and say "I didn't realize that"?

Don't just describe a bar chart of the fields. Think about what story this data tells that isn't obvious. What comparison, distribution, trend, or anomaly would be most revealing? What would look very different in a healthy system vs. a struggling one?

Respond naturally as a conversation starter — describe what you'd show and why it matters. No JSON, no bullet points, just your perspective.`;
...
const ALL_CATEGORIES = ["all", "llm", "security", "network", "infrastructure", "data", "system", "hardware", "automation"];

// ── FreePresentationCard ───────────────────────────────────────────────────

function FreePresentationCard({
    ...
    <Card className="flex flex-col h-full">
      ...
      <CardFooter className="pt-0">
        ...
        >
          ...
          {response ? "Ask again" : "What would Gemma show?"}
        </Button>
      </CardFooter>
    </Card>
  );
...
// ── GemmaFreePresentationPage ──────────────────────────────────────────────

export default function GemmaFreePresentationPage() {
  ...
  if (loading) {
    return (
      <div className="container mx-auto p-6 space-y-6">
        <h1 className="text-3xl font-bold flex items-center gap-2"><TrendingUp className="w-7 h-7 text-primary" /> Gemma Free Presentation</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {Array.from({ length: 8 }).map((_, i) => <Card key={i}><CardHeader><Skeleton className="h-5 w-32" /></CardHeader><CardContent><Skeleton className="h-20 w-full" /></CardContent></Card>)}
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            <TrendingUp className="w-7 h-7 text-primary" /> Gemma Free Presentation
          </h1>
          <p className="text-muted-foreground mt-1">
            Gemma describes what it would visualize and why — conversation starters, not charts
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <div className="mt-4">
          <ScrollArea className="h-[calc(100vh-320px)]">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filtered.map((card, i) => (
                <FreePresentationCard key={card.pluginId} {...card} autoGenerate={i < generateCount} />
              ))}
            </div>
          </ScrollArea>
        </div>
