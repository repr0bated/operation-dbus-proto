...
pub struct AntigravityChatState { pub status: String, pub bridge: Value, pub auth: Value, pub models: Value, pub config: Value }
pub struct AntigravityChatPlugin;
impl Default for AntigravityChatPlugin { fn default() -> Self { Self } }
...
impl StatePlugin for AntigravityChatPlugin {
    fn name(&self) -> &str { "antigravity_chat" }
    fn version(&self) -> &str { "1.0.0" }
    fn schema(&self) -> Option<PluginSchema> { Some(super::plugin_schema_defs::antigravity_chat_plugin_schema()) }
