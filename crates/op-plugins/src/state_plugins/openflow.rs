...
//
// Typed flow-mod types (OfpFlowMod, OfpMatch, OxmField, Action) are generated
// from schemas/openflow/openflow15.json via schemafy, which reads the official
// OpenFlow JSON schema (draft 4) and produces serde-serializable Rust structs.
// These generated types are the plugin's D-Bus method inputs — they get typed
// and sealed in the blob like rovs_commands. The schema covers OF1.3 plus
// OF1.5 extensions (encap, decap, packet_type) needed for the netmaker L3→L2
// encapsulation path.
//
// The types are regenerated by running:
//   cargo expand -p op-plugins --bin openflow_schema_gen > crates/op-plugins/src/state_plugins/openflow_generated.rs
// Then the schemafy dependency can be removed.
...
// Schema-generated types from schemas/openflow/openflow15.json via schemafy.
// Regenerate with: cargo expand -p op-plugins --bin openflow_schema_gen
// then extract the types into openflow_generated.rs.
#[path = "openflow_generated.rs"]
mod openflow_generated;
pub use openflow_generated::*;

/// Map a typed OxmFieldType enum variant to the string key used by the
/// internal FlowEntry.match_fields HashMap.
fn oxm_field_name(field: &OxmFieldType) -> &'static str {
    use OxmFieldType::*;
    match field {
        OfpxmtOfbInPort => "in_port",
        OfpxmtOfbInPhyPort => "in_phy_port",
        OfpxmtOfbMetadata => "metadata",
        OfpxmtOfbEthDst => "eth_dst",
        OfpxmtOfbEthSrc => "eth_src",
        OfpxmtOfbEthType => "eth_type",
        OfpxmtOfbVlanVid => "vlan_vid",
        OfpxmtOfbVlanPcp => "vlan_pcp",
        OfpxmtOfbIpDscp => "ip_dscp",
        OfpxmtOfbIpEcn => "ip_ecn",
        OfpxmtOfbIpProto => "ip_proto",
        OfpxmtOfbIpv4Src => "nw_src",
        OfpxmtOfbIpv4Dst => "nw_dst",
        OfpxmtOfbTcpSrc => "tcp_src",
        OfpxmtOfbTcpDst => "tcp_dst",
        OfpxmtOfbUdpSrc => "udp_src",
        OfpxmtOfbUdpDst => "udp_dst",
        OfpxmtOfbSctpSrc => "sctp_src",
        OfpxmtOfbSctpDst => "sctp_dst",
        OfpxmtOfbIcmpv4Type => "icmpv4_type",
        OfpxmtOfbIcmpv4Code => "icmpv4_code",
        OfpxmtOfbArpOp => "arp_op",
        OfpxmtOfbArpSpa => "arp_spa",
        OfpxmtOfbArpTpa => "arp_tpa",
        OfpxmtOfbArpSha => "arp_sha",
        OfpxmtOfbArpTha => "arp_tha",
        OfpxmtOfbIpv6Src => "ipv6_src",
        OfpxmtOfbIpv6Dst => "ipv6_dst",
        OfpxmtOfbIpv6Flabel => "ipv6_flabel",
        OfpxmtOfbIcmpv6Type => "icmpv6_type",
        OfpxmtOfbIcmpv6Code => "icmpv6_code",
        OfpxmtOfbIpv6NdTarget => "ipv6_nd_target",
        OfpxmtOfbIpv6NdSll => "ipv6_nd_sll",
        OfpxmtOfbIpv6NdTll => "ipv6_nd_tll",
        OfpxmtOfbMplsLabel => "mpls_label",
        OfpxmtOfbMplsTc => "mpls_tc",
        OfpxmtOfbMplsBos => "mpls_bos",
        OfpxmtOfbPbbIsid => "pbb_isid",
        OfpxmtOfbTunnelId => "tunnel_id",
        OfpxmtOfbIpv6Exthdr => "ipv6_exthdr",
        OfpxmtOfbPacketType => "packet_type",
    }
}

/// Convert a schema-generated Action to the internal FlowAction enum.
fn action_from_generated(a: &Action) -> FlowAction {
    use ActionType::*;
    match a.type_ {
        OfpatOutput => FlowAction::Output {
            port: a.port.clone().unwrap_or_default(),
        },
        OfpatSetField => FlowAction::SetField {
            field: a.field.clone().unwrap_or_default(),
            value: a.value.clone().unwrap_or_default(),
        },
        OfpatGroup => FlowAction::Output {
            port: a.group_id.map(|g| g.to_string()).unwrap_or_default(),
        },
        OfpatEncap => FlowAction::Encap {
            ether_type: a.ether_type.clone().unwrap_or_else(|| "ethernet".to_string()),
        },
        OfpatDecap => FlowAction::Decap,
        _ => FlowAction::Drop,
    }
}
...
pub enum FlowAction {
    ...
    /// Encapsulate a packet with a new header (OF1.5).
    /// Used by the netmaker L3->L2 path: pure L3 packets (no Ethernet
    /// header) arriving on the WG port get an Ethernet encap before
    /// being handed to the host stack via LOCAL.
    Encap { ether_type: String },

    /// Decapsulate a packet (OF1.5).
    Decap,
}
...
impl OpenFlowPlugin {
    ...
    fn parse_actions(&self, actions_str: &str) -> Vec<FlowAction> {
        ...
        for action in actions_str.split(',') {
            ...
            } else if let Some(rest) = action.strip_prefix("encap(") {
                if let Some(et) = rest.strip_suffix(')') {
                    actions.push(FlowAction::Encap {
                        ether_type: et.to_string(),
                    });
                }
            } else if action == "decap" {
                actions.push(FlowAction::Decap);
            }
        }
    ...
    fn action_to_string(&self, action: &FlowAction) -> String {
        match action {
            ...
            FlowAction::Encap { ether_type } => format!("encap({})", ether_type),
            FlowAction::Decap => "decap".to_string(),
...
impl StatePlugin for OpenFlowPlugin {
    ...
    fn is_available(&self) -> bool {
        let sock = std::env::var("OVSDB_SOCK")
            .unwrap_or_else(|_| "/usr/local/var/run/openvswitch/db.sock".to_string());
    ...
    fn unavailable_reason(&self) -> String {
        let sock = std::env::var("OVSDB_SOCK")
            .unwrap_or_else(|_| "/usr/local/var/run/openvswitch/db.sock".to_string());
        format!("OpenFlow requires OVSDB socket at {} (OVSDB_SOCK env)", sock)
    }
    ...
    async fn call_method(&self, method: &str, args: &serde_json::Value) -> Result<serde_json::Value> {
        // All three methods use the schema-generated OpenFlowFlowModInput.
        // The method name determines the command; the flow_mod.command field
        // from the input is used to override if explicitly set.
        let input: OpenFlowFlowModInput = serde_json::from_value(args.clone())
            .context("parse openflow method arguments")?;
        let fm = &input.flow_mod;

        // Convert typed OXM match fields to internal FlowEntry format.
        let mut match_fields = HashMap::new();
        if let Some(oxm_fields) = &fm.match_.oxm_fields {
            for f in oxm_fields {
                let key = oxm_field_name(&f.oxm_field);
                if let Some(val) = &f.oxm_value {
                    match_fields.insert(key.to_string(), val.clone());
                } else {
                    match_fields.insert(key.to_string(), String::new());
                }
            }
        }

        // Convert typed actions to internal FlowAction format.
        let actions: Vec<FlowAction> = fm.actions.as_deref().unwrap_or(&[])
            .iter().map(action_from_generated).collect();

        let flow = FlowEntry {
            table: fm.table_id as u8,
            priority: fm.priority as u16,
            match_fields,
            actions,
            cookie: fm.cookie.as_ref().and_then(|c| u64::from_str_radix(c, 16).ok()),
            idle_timeout: fm.idle_timeout as u16,
            hard_timeout: fm.hard_timeout as u16,
        };
        ...
        match method {
            ...
            "delete_flow" => {
                ...
                for existing in flows {
                    let matches = flow.match_fields.iter().all(|(k, v)| {
                        existing.match_fields.get(k).map(|s| s.as_str()) == Some(v.as_str())
                    });
                    if matches {
                        self.delete_flow(&input.bridge, &existing).await
            ...
            "modify_flow" => {
                let flows = self.query_flows(&input.bridge).await
                    .context("query current flows")?;
                for mut existing in flows {
                    let matches = flow.match_fields.iter().all(|(k, v)| {
                        existing.match_fields.get(k).map(|s| s.as_str()) == Some(v.as_str())
                    });
                    if matches {
                        existing.priority = flow.priority;
                        existing.actions = flow.actions.clone();
                        if let Err(e) = self.delete_flow(&input.bridge, &existing).await {
                            log::warn!("failed to delete old flow before modify: {e}");
                        }
                        self.install_flow(&input.bridge, &existing).await
...
pub(crate) fn openflow_schema() -> PluginSchema {
    ...
    // D-Bus methods: all three flow operations use the schema-generated
    // OpenFlowFlowModInput (typed OfpFlowMod with OXM match fields and
    // typed Action list). The command field distinguishes add/delete/modify.
    // These typed methods are sealed into the blob on reseal.
    schema.methods.insert(
        "add_flow".to_string(),
        super::plugin_scaffold_helpers::method_decl_from_schemars_with_output::<
            OpenFlowFlowModInput,
