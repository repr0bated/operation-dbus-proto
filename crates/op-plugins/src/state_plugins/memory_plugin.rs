...
pub struct MemoryState { pub status: String, pub backend: String, pub namespaces: Value, pub stats: Value, pub config: Value }
pub struct MemoryPlugin;
impl Default for MemoryPlugin { fn default() -> Self { Self } }
impl MemoryPlugin {
    pub fn new() -> Self { Self }
    pub(crate) fn current_state() -> MemoryState {
        MemoryState {
            status: "active".to_string(), backend: "sqlite".to_string(),
...
impl StatePlugin for MemoryPlugin {
    ...
    fn schema(&self) -> Option<PluginSchema> { Some(super::plugin_schema_defs::memory_plugin_schema()) }
