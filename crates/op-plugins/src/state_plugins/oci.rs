...
use op_state_store::{CapabilityDecl, Constraint, FieldSchema, FieldType, PluginSchema, ReadOnlyCondition};
...
pub(crate) fn oci_schema() -> PluginSchema {
    let mut schema = PluginSchema::builder("oci")
        .category("service")
        .version("1.0.0")
        .category("container")
    ...
    #[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
    struct OciInspectorSchema {
        #[serde(default)]
        #[schemars(extend("x-oscal-subid" = "sch.software.plugin.oci.inspector-fields@v1"))]
        inspector_fields: inspector_gadget_generated::InspectorGadgetFields,
    }
    let root = serde_json::to_value(schemars::schema_for!(OciInspectorSchema))
        .expect("OCI Inspector schema serializes to JSON");
    let mut inspector = super::schemars_adapter::plugin_schema_from_json(
        "oci_inspector",
        "1.0.0",
        "OCI upstream Inspector fields",
        &root,
    );
