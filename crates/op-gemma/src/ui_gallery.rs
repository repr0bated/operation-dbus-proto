...
pub fn generate_gallery() -> Result<()> {
    ...
    info!(existing = existing.specs.len(), "Gemma UI gallery: existing specs loaded");
    let used_signatures: HashSet<String> = existing.specs.iter().map(|s| s.signature.clone()).collect();
    ...
    while gallery.specs.len() < TARGET_COUNT && attempts < max_attempts {
        ...
        let root_id = spec.get("root").and_then(|r| r.as_str()).unwrap_or("").to_string();
    ...
    info!(total = gallery.specs.len(), "Gemma UI gallery written to /dev/shm/gemma-ui-specs.json");
    Ok(())
}
...
pub fn delete_and_replace(spec_id: &str) -> Result<()> {
    ...
    let plugins = load_plugin_schemas()?;
    let used_signatures: HashSet<String> = gallery.specs.iter().map(|s| s.signature.clone()).collect();
    let generators = all_generators();
        ...
        let mut elem = element.clone();
            if let Some(obj) = elem.as_object_mut() {
                if !children.is_empty() {
                    obj.insert("children".to_string(), json!(children.iter().map(|c| *c).collect::<Vec<_>>()));
                } else {
...
fn gen_live_logs(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    ...
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "Live System Logs", "description": "Real-time D-Bus and service logs"})), vec!["btn-1", "log-1"]),
        ("btn-1", el("Button", json!({"label": "Clear", "variant": "outline"})), vec![]),
        ("log-1", el("LogStream", json!({"title": null, "lines": log_lines, "autoScroll": true, "maxHeight": "500px"})), vec![]),
    ])
}
...
fn gen_procfs_dashboard(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let cpus = (n % 8) + 1;
    spec("grid-1", vec![
        ("grid-1", el("Grid", json!({"columns": 3, "gap": "md"})), vec!["card-cpu", "card-mem", "card-load"]),
        ("card-cpu", el("Card", json!({"title": "CPU Usage"})), vec!["m-cpu1", "m-cpu2", "m-cpu3"]),
        ("m-cpu1", el("ProcfsMetric", json!({"label": "CPU 0", "value": 45 + (n % 30), "unit": "%", "warningThreshold": 70, "criticalThreshold": 90})), vec![]),
        ("m-cpu2", el("ProcfsMetric", json!({"label": "CPU 1", "value": 62 + (n % 20), "unit": "%", "warningThreshold": 70, "criticalThreshold": 90})), vec![]),
        ("m-cpu3", el("ProcfsMetric", json!({"label": "CPU 2", "value": 28 + (n % 40), "unit": "%", "warningThreshold": 70, "criticalThreshold": 90})), vec![]),
        ("card-mem", el("Card", json!({"title": "Memory"})), vec!["m-mem1", "m-mem2"]),
        ("m-mem1", el("ProcfsMetric", json!({"label": "Used", "value": 7.2, "unit": "GB", "warningThreshold": 12, "criticalThreshold": 15})), vec![]),
        ("m-mem2", el("ProcfsMetric", json!({"label": "Cached", "value": 3.4, "unit": "GB", "warningThreshold": 999, "criticalThreshold": 999})), vec![]),
        ("card-load", el("Card", json!({"title": "System Load"})), vec!["m-load1", "m-up"]),
        ("m-load1", el("ProcfsMetric", json!({"label": "Load avg (1m)", "value": (cpus as f64 * 0.3 + (n % 10) as f64 * 0.1), "unit": "", "warningThreshold": cpus as f64 * 0.7, "criticalThreshold": cpus as f64})), vec![]),
        ("m-up", el("KVPair", json!({"key": "Uptime", "value": format!("{}d {}h {}m", n % 30, n % 24, n % 60), "mono": true})), vec![]),
    ])
}

fn gen_agent_config(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let agents = ["rust_pro", "memory", "sequential_thinking", "backend_architect", "context_manager", "code_reviewer"];
    let agent = agents[n % agents.len()];
    let tools = ["ovs_create_bridge", "ovs_add_port", "llm_chat", "tool_execute", "state_query", "service_restart", "file_read", "code_search"];
    let selected_tools: Vec<String> = tools.iter().skip(n % 3).take(3 + (n % 3)).map(|t| t.to_string()).collect();

    spec("card-1", vec![
        ("card-1", el("AgentConfigForm", json!({
            "agentName": agent,
        ...
        ("btn-row", el("ButtonGroup", json!({"buttons": [
...
fn gen_flow_table(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let flows: Vec<Value> = (0..12).map(|i| {
    ...
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "OpenFlow Flow Table", "description": "ovsbr0 active flows"})), vec!["ft-1", "btn-1"]),
        ("ft-1", el("FlowTable", json!({"title": null, "flows": flows})), vec![]),
        ("btn-1", el("Button", json!({"label": "Add Flow", "variant": "outline"})), vec![]),
    ])
}

fn gen_network_topology(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let nodes: Vec<Value> = [
        ("host", "ens3", "host", "up"), ("bridge", "ovsbr0", "bridge", "up"),
        ("port", "ovsbr0-mgmt", "port", "up"), ("port", "ovsbr0-sock", "port", "up"),
        ("tunnel", "gbr-wg", "tunnel", "up"), ("tunnel", "gbr-warp", "tunnel", "up"),
        ("tunnel", "gbr-xray", "tunnel", "up"), ("container", "qdrant", "container", "up"),
        ("container", "netmaker-ui", "container", "up"), ("container", "assistant", "container", "up"),
    ].iter().enumerate().map(|(i, (id, label, ty, st))| json!({
        "id": id, "label": label, "type": ty, "status": if i + n < 8 { st } else { "down" },
    })).collect();

    let edges: Vec<Value> = [
        ("host", "bridge", "uplink"), ("bridge", "port", "mgmt"),
        ("bridge", "port", "sock"), ("bridge", "tunnel", "wg"),
        ("bridge", "tunnel", "warp"), ("bridge", "tunnel", "xray"),
        ("port", "container", "qdrant"), ("port", "container", "assistant"),
    ].iter().map(|(f, t, l)| json!({"from": f, "to": t, "label": l})).collect();

    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "Network Topology", "description": "OVS bridge and connected ports"})), vec!["nt-1"]),
        ("nt-1", el("NetworkTopology", json!({"nodes": nodes, "edges": edges})), vec![]),
    ])
}
...
fn gen_plugin_state(plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    ...
    let field_elements: Vec<Value> = schema.fields.iter().take(8).enumerate().map(|(i, (fname, fval))| {
        let ft = fval.get("field_type").and_then(|v| v.as_str()).unwrap_or("string");
        let desc = fval.get("description").and_then(|v| v.as_str()).unwrap_or("");
        let ro = fval.get("read_only").and_then(|v| v.as_bool()).unwrap_or(false);
        let req = fval.get("required").and_then(|v| v.as_bool()).unwrap_or(false);
        let ft_str = if ft == "any" { "string" } else if ft.starts_with('{') { "object" } else { ft };
        json!({
            "name": fname,
            "fieldType": ft_str,
            "description": desc,
    ...
    for (i, fe) in field_elements.iter().enumerate() {
        elements.push((leak(format!("sf-{i}")), el("SchemaField", fe.clone()), vec![]));
    }
    elements.push((
        "btn-row",
        el("ButtonGroup", json!({"buttons": [
            {"label": "Save", "variant": "default"},
...
fn gen_service_manager(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let services = [
        ("op-dbus", "active", 86400, 0), ("op-web-srv", "active", 86400, 0),
        ("gbr-xray", "active", 86400, 0), ("nextdns-srv", "active", 86400, 0),
        ("zeroclaw", "active", 3600, 0), ("op-cognitive-mcp", "active", 7200, 0),
        ("op-grpc-bridge-zeroclaw", "active", 86400, 0), ("incus", "active", 86400, 0),
        ("netmaker", "inactive", 0, 3), ("wg-xray", "inactive", 0, 1),
    ];
    ...
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "s6 Service Manager", "description": "All supervised services"})), vec!["tbl-1"]),
        ("tbl-1", el("Table", json!({
...
fn gen_audit_timeline(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    ...
    spec("acc-1", vec![
        ("acc-1", el("Accordion", json!({"items": [
            {"title": "Today", "value": "today"},
            {"title": "Yesterday", "value": "yesterday"},
        ], "type": "single", "defaultValue": "today"})), vec![]),
        ("today-content", el("Stack", json!({"direction": "vertical", "gap": "sm"})),
            (0..15).map(|i| leak(format!("te-{i}"))).collect()),
    ]);

    // Build properly
    let mut elements: Vec<(&str, Value, Vec<&str>)> = Vec::new();
    let te_ids: Vec<&str> = (0..15).map(|i| {
        let s = format!("te-{i}");
        leak(s)
    }).collect();

    elements.push(("card-1", el("Card", json!({"title": "Audit Timeline", "description": "Hash-linked event chain"})), te_ids.clone().into()));
...
fn gen_schema_gallery(plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    ...
    let cards: Vec<Value> = names.iter().enumerate().map(|(i, name)| {
    ...
    let card_ids: Vec<&str> = (0..cards.len()).map(|i| {
        let s = format!("pc-{i}");
        leak(s)
    }).collect();

    let mut elements: Vec<(&str, Value, Vec<&str>)> = Vec::new();
    elements.push(("grid-1", el("Grid", json!({"columns": 4, "gap": "sm"})), card_ids.clone().into()));
...
fn gen_ovs_bridges(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("card-1", vec![
        ...
        ("sep-1", el("Separator", json!({})), vec![]),
        ("ft-1", el("FlowTable", json!({"title": "Active Flows", "flows": [
...
fn gen_privacy_chain(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("stack-1", vec![
        ("stack-1", el("Stack", json!({"direction": "vertical", "gap": "md"})), vec!["h-1", "nt-1", "alert-1"]),
        ("h-1", el("Heading", json!({"level": "h2", "text": "Privacy Chain: WG → WARP → Xray"})), vec![]),
        ("nt-1", el("NetworkTopology", json!({
            "nodes": [
        ...
        ("alert-1", el("Alert", json!({"title": "Chain Active", "message": "All 3 privacy tunnels are up and forwarding", "type": "info"})), vec![]),
    ])
}
...
fn gen_oscal_registry(_plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    ...
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "OSCAL Subid Registry", "description": "Canonical taxonomy"})), vec!["tbl-1"]),
        ("tbl-1", el("Table", json!({"columns": ["Subid", "Category", "Status"], "rows": entries})), vec![]),
    ])
}
...
fn gen_mixed_dashboard(plugins: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    ...
    match variant {
        0 => {
            ...
            spec("grid-1", vec![
                ("grid-1", el("Grid", json!({"columns": 2, "gap": "md"})), vec!["log-1", "metrics-1"]),
                ("log-1", el("LogStream", json!({"title": "D-Bus Events", "lines": [
                ...
                ("metrics-1", el("Stack", json!({"direction": "vertical", "gap": "sm"})), vec!["m-1", "m-2"]),
                ("m-1", el("ProcfsMetric", json!({"label": "CPU", "value": 34, "unit": "%", "warningThreshold": 70, "criticalThreshold": 90})), vec![]),
                ("m-2", el("ProcfsMetric", json!({"label": "Memory", "value": 7.2, "unit": "GB", "warningThreshold": 12, "criticalThreshold": 15})), vec![]),
            ])
        }
        1 => {
            // Topology + flow table in tabs
            spec("tabs-1", vec![
                ("tabs-1", el("Tabs", json!({"tabs": [{"label": "Topology", "value": "topo"}, {"label": "Flows", "value": "flows"}], "defaultValue": "topo"})), vec![]),
            ])
        }
        2 => {
            // Agent grid
            ...
            spec("grid-1", vec![
                ("grid-1", el("Grid", json!({"columns": 3, "gap": "sm"})), ids.to_vec()),
        ...
        _ => {
            ...
            spec("stack-1", vec![
                ("stack-1", el("Stack", json!({"direction": "vertical", "gap": "md"})), vec!["de-1", "te-1"]),
                ("de-1", el("DataExplorer", json!({"title": "Plugin State", "data": json!({"openflow": {"flows": 42}, "wireguard": {"peers": 3}}).to_string()})), vec![]),
                ("te-1", el("TimelineEvent", json!({"timestamp": "22:00:00", "event_type": "mut", "title": "State mutation", "description": "openflow.flows updated", "severity": "info"})), vec![]),
            ])
        }
    }
}
...
fn gen_nested_dialog(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let titles = ["Configuration", "Settings", "Preferences", "Advanced Options", "Edit Profile", "Network Setup", "Security Config"];
    let labels = ["Open Settings", "Configure", "Edit", "Modify", "Adjust", "Setup", "Tune"];
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": titles[n % titles.len()]})), vec!["btn-1", "txt-1"]),
        ("btn-1", el("Button", json!({"label": labels[n % labels.len()], "variant": match n % 3 { 0 => "default", 1 => "outline", _ => "secondary" }})), vec![]),
        ("txt-1", el("Text", json!({"content": format!("Click to open {} dialog", titles[n % titles.len()]), "variant": "caption"})), vec![]),
    ])
}

fn gen_carousel_badges(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let all_badges = ["online", "encrypted", "audited", "paired", "synced", "healthy", "active", "secure", "verified", "trusted"];
    let count = 3 + (n % 4);
    ...
    spec("carousel-1", vec![
        ("carousel-1", el("Carousel", json!({"items": badges})), vec![]),
    ])
}
...
fn gen_accordion_logs(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("acc-1", vec![
        ...
        ], "type": "multiple", "defaultValue": "dbus"})), vec![]),
    ])
}

fn gen_drawer_topology(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("stack-1", vec![
        ("stack-1", el("Stack", json!({"direction": "vertical", "gap": "sm"})), vec!["btn-1", "nt-1"]),
        ("btn-1", el("Button", json!({"label": "Show Network Map", "variant": "outline"})), vec![]),
        ("nt-1", el("NetworkTopology", json!({
...
fn gen_popover_metrics(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("row-1", vec![
        ("row-1", el("Stack", json!({"direction": "horizontal", "gap": "sm"})), vec!["b-1", "b-2"]),
        ("b-1", el("Badge", json!({"text": "CPU: 34%", "variant": "success"})), vec![]),
        ("b-2", el("Badge", json!({"text": "MEM: 7.2GB", "variant": "warning"})), vec![]),
    ])
}

fn gen_toggle_filter_table(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("stack-1", vec![
        ("stack-1", el("Stack", json!({"direction": "vertical", "gap": "sm"})), vec!["tg-1", "tbl-1"]),
        ("tg-1", el("ToggleGroup", json!({"items": ["All", "Active", "Inactive", "Error"], "type": "single", "value": "All"})), vec![]),
        ("tbl-1", el("Table", json!({"columns": ["Service", "Status"], "rows": [{"name": "op-dbus", "status": "active"}]})), vec![]),
    ])
}

fn gen_slider_thresholds(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "Alert Thresholds"})), vec!["s-1", "s-2", "s-3"]),
        ("s-1", el("Slider", json!({"label": "CPU Warning", "min": 0, "max": 100, "step": 1, "value": 70})), vec![]),
        ("s-2", el("Slider", json!({"label": "CPU Critical", "min": 0, "max": 100, "step": 1, "value": 90})), vec![]),
        ("s-3", el("Slider", json!({"label": "Memory Warning (GB)", "min": 0, "max": 32, "step": 1, "value": 12})), vec![]),
    ])
}
...
fn gen_collapsible_tree(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    ...
    spec("col-1", vec![
        ("col-1", el("Collapsible", json!({"title": "System State Tree", "defaultOpen": true})), vec!["de-1"]),
        ("de-1", el("DataExplorer", json!({"title": null, "data": data, "searchQuery": null})), vec![]),
    ])
}

fn gen_tabs_config(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "System Configuration"})), vec!["tabs-1"]),
        ("tabs-1", el("Tabs", json!({"tabs": [
        ...
        ], "defaultValue": "general"})), vec![]),
    ])
}

fn gen_kv_inspector(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let pairs: Vec<Value> = [
        ("hostname", "ghostbridge"), ("kernel", "7.0.11-xanmod1"),
        ("public_ip", "148.113.204.83"), ("ovsbr0_ip", "10.200.0.1/30"),
        ("xray_port", "443"), ("grpc_port", "50051"),
        ("nextdns_profile", "689ec7"), ("dbus_bus", "org.opdbus.v1.plugins"),
    ].iter().map(|(k, v)| json!({"key": k, "value": v, "mono": true})).collect();

    let ids: Vec<&str> = (0..pairs.len()).map(|i| {
        let s = format!("kv-{i}");
        leak(s)
    }).collect();

    let mut elements: Vec<(&str, Value, Vec<&str>)> = Vec::new();
    elements.push(("card-1", el("Card", json!({"title": "System Inspector"})), ids.clone().into()));
...
fn gen_button_group_actions(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "Service Control"})), vec!["bg-1"]),
        ("bg-1", el("ButtonGroup", json!({"buttons": [
...
fn gen_progress_ring(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("grid-1", vec![
        ("grid-1", el("Grid", json!({"columns": 3, "gap": "md"})), vec!["p-1", "p-2", "p-3"]),
        ("p-1", el("Progress", json!({"value": 34, "max": 100, "label": "CPU"})), vec![]),
        ("p-2", el("Progress", json!({"value": 72, "max": 100, "label": "Memory"})), vec![]),
        ("p-3", el("Progress", json!({"value": 15, "max": 100, "label": "Disk"})), vec![]),
    ])
}

fn gen_skeleton_loader(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "Loading Plugin State..."})), vec!["s-1", "s-2", "s-3"]),
        ("s-1", el("Skeleton", json!({"width": "100%", "height": "20px", "rounded": true})), vec![]),
        ("s-2", el("Skeleton", json!({"width": "80%", "height": "20px", "rounded": true})), vec![]),
        ("s-3", el("Skeleton", json!({"width": "60%", "height": "20px", "rounded": true})), vec![]),
    ])
}
...
fn gen_alert_banner(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    ...
    let msgs = [
        ...
        ("High Memory", "Memory usage at 72% — approaching warning threshold"),
        ("Service Failed", "netmaker container crash-looping on LXC mount error"),
        ("Update Complete", "All plugins schemas re-projected successfully"),
    ];
    let (title, msg) = msgs[n % msgs.len()];
    spec("stack-1", vec![
        ("stack-1", el("Stack", json!({"direction": "vertical", "gap": "sm"})), vec!["alert-1"]),
        ("alert-1", el("Alert", json!({"title": title, "message": msg, "type": types[n % types.len()]})), vec![]),
    ])
}

fn gen_dropdown_menu_actions(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "Quick Actions"})), vec!["dm-1"]),
        ("dm-1", el("DropdownMenu", json!({"label": "Actions", "items": ["Restart Service", "View Logs", "Edit Config", "Delete"]})), vec![]),
    ])
}

fn gen_pagination_table(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let rows: Vec<Value> = (0..20).map(|i| json!({
    ...
    spec("stack-1", vec![
        ("stack-1", el("Stack", json!({"direction": "vertical", "gap": "sm"})), vec!["tbl-1", "pg-1"]),
        ("tbl-1", el("Table", json!({"columns": ["ID", "Type", "Timestamp"], "rows": rows})), vec![]),
        ("pg-1", el("Pagination", json!({"totalPages": 10, "page": 1})), vec![]),
    ])
}

fn gen_grid_metrics(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let cols = (n % 4) + 2;
    let ids: Vec<&str> = (0..cols * 2).map(|i| {
        let s = format!("m-{i}");
        leak(s)
    }).collect();

    let mut elements: Vec<(&str, Value, Vec<&str>)> = Vec::new();
    elements.push(("grid-1", el("Grid", json!({"columns": cols, "gap": "sm"})), ids.clone().into()));
    for i in 0..cols * 2 {
        elements.push((ids[i], el("ProcfsMetric", json!({
...
fn gen_stack_forms(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("stack-1", vec![
        ("stack-1", el("Stack", json!({"direction": "vertical", "gap": "md"})), vec!["inp-1", "inp-2", "inp-3", "btn-1"]),
        ("inp-1", el("Input", json!({"label": "Hostname", "name": "hostname", "placeholder": "ghostbridge", "type": "text"})), vec![]),
        ("inp-2", el("Input", json!({"label": "Port", "name": "port", "placeholder": "50051", "type": "number"})), vec![]),
        ("inp-3", el("Select", json!({"label": "Protocol", "name": "protocol", "options": ["grpc", "tcp", "unix"], "value": "grpc"})), vec![]),
        ("btn-1", el("Button", json!({"label": "Save", "variant": "default"})), vec![]),
    ])
}

fn gen_separator_sections(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("stack-1", vec![
        ("stack-1", el("Stack", json!({"direction": "vertical", "gap": "md"})), vec!["h-1", "txt-1", "sep-1", "h-2", "txt-2"]),
        ("h-1", el("Heading", json!({"level": "h3", "text": "Network"})), vec![]),
        ("txt-1", el("Text", json!({"content": "OVS bridge ovsbr0 with 6 ports and 42 flows", "variant": "body"})), vec![]),
        ("sep-1", el("Separator", json!({})), vec![]),
        ("h-2", el("Heading", json!({"level": "h3", "text": "Services"})), vec![]),
        ("txt-2", el("Text", json!({"content": "12 active, 3 inactive, 0 failed", "variant": "body"})), vec![]),
    ])
}

fn gen_radio_provider(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "LLM Provider"})), vec!["r-1"]),
        ("r-1", el("Radio", json!({"label": "Select provider", "name": "provider", "options": ["anthropic", "openai", "google", "ollama", "factory"], "value": "anthropic"})), vec![]),
    ])
}

fn gen_checkbox_tools(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let tools = ["ovs_create_bridge", "ovs_add_port", "llm_chat", "tool_execute", "state_query", "service_restart"];
    let ids: Vec<&str> = (0..tools.len()).map(|i| {
        let s = format!("cb-{i}");
        leak(s)
    }).collect();

    let mut elements: Vec<(&str, Value, Vec<&str>)> = Vec::new();
    elements.push(("card-1", el("Card", json!({"title": "Agent Tool Assignment"})), ids.clone().into()));
    for (i, tool) in tools.iter().enumerate() {
        elements.push((ids[i], el("Checkbox", json!({"label": tool, "name": tool, "checked": (i + n) % 2 == 0})), vec![]));
    }
...
fn gen_textarea_prompt(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("card-1", vec![
        ("card-1", el("Card", json!({"title": "System Prompt Editor"})), vec!["ta-1", "btn-1"]),
        ("ta-1", el("Textarea", json!({"label": "System Prompt", "name": "prompt", "placeholder": "You are a helpful AI assistant...", "rows": 10, "value": "You are ANNA, the Axon Network Notary Arbitrator. You notarize WireGuard identity and handle Snowball sessions."})), vec![]),
        ("btn-1", el("Button", json!({"label": "Save Prompt", "variant": "default"})), vec![]),
    ])
}

fn gen_link_nav(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    spec("stack-1", vec![
        ("stack-1", el("Stack", json!({"direction": "horizontal", "gap": "md"})), vec!["l-1", "l-2", "l-3", "l-4"]),
        ("l-1", el("Link", json!({"label": "Overview", "href": "/"})), vec![]),
        ("l-2", el("Link", json!({"label": "Logs", "href": "/logs"})), vec![]),
        ("l-3", el("Link", json!({"label": "Services", "href": "/services"})), vec![]),
        ("l-4", el("Link", json!({"label": "Gallery", "href": "/gemma-gallery"})), vec![]),
    ])
}

fn gen_avatar_agents(_p: &HashMap<String, Vec<PluginSchema>>, n: usize) -> Value {
    let agents = [("ANNA", "scribe"), ("Olivia", "scal"), ("Eugene", "risk"), ("Penny", "privacy"), ("Reggie", "opa")];
    let ids: Vec<&str> = (0..agents.len()).map(|i| {
        let s = format!("av-{i}");
        leak(s)
    }).collect();

    let mut elements: Vec<(&str, Value, Vec<&str>)> = Vec::new();
    elements.push(("grid-1", el("Grid", json!({"columns": 5, "gap": "sm"})), ids.clone().into()));
    for (i, (name, role)) in agents.iter().enumerate() {
        elements.push((ids[i], el("Avatar", json!({"src": null, "name": name, "size": "md"})), vec![]));
    }
...
fn load_plugin_schemas() -> Result<HashMap<String, Vec<PluginSchema>>> {
    ...
    let parsed: PluginSchemaCatalog = serde_json::from_str(&raw)
        .with_context(|| "failed to parse live schema")?;
    Ok(parsed.plugins)
}
...
fn load_existing_specs() -> Result<GemmaSpecGallery> {
    match fs::read_to_string(GEMMA_UI_SPECS) {
        Ok(raw) => {
            let gallery: GemmaSpecGallery = serde_json::from_str(&raw)
                .unwrap_or(GemmaSpecGallery { version: 1, specs: vec![] });
            Ok(gallery)
        }
        Err(_) => Ok(GemmaSpecGallery { version: 1, specs: vec![] }),
    }
}
    ...
    if let Some(elements) = spec.get("elements").and_then(|e| e.as_object()) {
        ...
        fn walk(id: &str, elements: &Map<String, Value>, sig: &mut String, depth: usize) {
            if let Some(elem) = elements.get(id) {
                ...
                sig.push_str(&format!("{}:{}:{}|", depth, ty, &props_hash[..8.min(props_hash.len())]));
