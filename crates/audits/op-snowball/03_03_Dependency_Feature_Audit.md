...
The following table inventories all direct dependencies declared in `crates/op-snowball/Cargo.toml` and resolved via the workspace `Cargo.toml`.
...
The `op-snowball` crate defines the following `[features]` block in `crates/op-snowball/Cargo.toml`:
...
*   **`ml`**: Enables transformer-based vectorization pipelines. 
    *   **Gated Blocks**: Used in `crates/op-snowball/src/plugin_footprint.rs` under `#[cfg(feature = "ml")]` at lines 112, 126, and 144 to invoke the global `ModelManager` and run semantic embedding logic.
...
*   **Protocol Buffer Integration**: Absent. There are no dependencies on `prost`, `tonic`, or schema generation tools in `crates/op-snowball/Cargo.toml`.
*   **Ad-hoc Struct Definitions**: The primary data models, `BlockEvent` and `PluginFootprint`, are declared directly as localized Rust structs in two separate locations:
    *   `crates/op-snowball/src/footprint.rs:10` and `footprint.rs:43`
    *   `crates/op-snowball/src/plugin_footprint.rs:11`
*   **Duplicate Structures & Type Inconsistencies**: The two definitions of `PluginFootprint` are syntactically duplicated across separate files inside the same crate. `prelude` module exports are forced to alias `plugin_footprint::PluginFootprint` as `LegacyPluginFootprint` (in `crates/op-snowball/src/lib.rs:24`), indicating an unresolved migration path.
...
The storage backend usage has been scanned across all provided source files of `op-snowball`.

| Backend | Found at File:Line | Role | Architectural Match |
| :--- | :--- | :--- | :--- |
| **BTRFS Subvolumes (FS-based)** | `crates/op-snowball/src/snowball.rs:47-49` | Dual-subvolume layout representing raw event timings, vector projections, and current states. | **Yes** (Matches subvolume specification) |
| **JSON Plain Files** | `crates/op-snowball/src/btrfs_numa_integration.rs:118` | Writes block event metadata under `by-hash` inside BTRFS subvolume paths. | **Partial** (Lacks formal indexing DB) |
| **Binary Vector Slices** | `crates/op-snowball/src/snowball.rs:163` | Raw vector arrays written to disk as `.bin` or `.vec` files. | **No** (Expected unified vector storage like CozoDB/Qdrant) |
...
#### Code Locations
*   `crates/op-snowball/src/snowball.rs:245-251`:
    ```rust
    ...
    ```
*   `crates/op-snowball/src/streaming_snowball.rs:475-481`:
    ```rust
    ...
    ```
*   `crates/op-snowball/src/streaming_snowball.rs:512-516`:
    ```rust
    let cmd = format!(
...
#### Code Locations
*   `crates/op-snowball/src/snowball.rs:229-231`:
    ```rust
    ...
    ```
*   `crates/op-snowball/src/streaming_snowball.rs:608-610`:
    ```rust
...
#### Description
When updating the current system state or writing granular plugin configurations, the snowball writer places temporary state updates in predictable, static paths before atomically renaming them.

#### Code Locations
*   `crates/op-snowball/src/streaming_snowball.rs:323`:
    ```rust
    let temp_file = self.state_subvol.join(".current.json.tmp");
    tokio::fs::write(&temp_file, simd_json::to_string_pretty(state)?).await?;
    ```
*   `crates/op-snowball/src/streaming_snowball.rs:335`:
    ```rust
    let temp_file = plugins_dir.join(format!(".{}.json.tmp", plugin_name));
    ```

#### Exploitation Vector
If `state_subvol` or `plugins_dir` exists in a shared, world-writable directory (such as `/tmp` or a shared volume path), a local unprivileged attacker can create a symbolic link at `/var/lib/snowball/state/.current.json.tmp` targeting an arbitrary file on the system (e.g., `/etc/shadow` or `/etc/cron.d/malicious`). When the snowball control plane updates system state, it writes the configuration payloads into the symlinked destination, enabling arbitrary file overwrite and privilege escalation.
...
#### Description
The timing snowball marks timing data as authoritative and uses this value to calculate cryptographic block hashes. However, the system relies strictly on unvalidated system time, allowing the sequence of block hashes to be easily compromised.

#### Code Locations
*   `crates/op-snowball/src/footprint.rs:24-29`:
    ```rust
    ...
    ```
*   `crates/op-snowball/src/plugin_footprint.rs:100-101`:
    ```rust
...
#### Code Locations
*   `crates/op-snowball/src/btrfs_numa_integration.rs:150-151`:
    ```rust
    ...
    ```
*   `crates/op-snowball/src/snowball.rs:213-215`:
    ```rust
    let mut data = tokio::fs::read_to_string(&state_file).await?;
    Ok(unsafe { simd_json::from_str(&mut data)? })
    ```
*   `crates/op-snowball/src/streaming_snowball.rs:344-346`:
    ```rust
...
#### Code Location
*   `crates/op-snowball/src/btrfs_numa_integration.rs:137-147`:
    ```rust
...
#### Code Locations
*   `crates/op-snowball/src/snowball.rs:125-131`:
    ```rust
    ...
    ```
*   `crates/op-snowball/src/snowball.rs:207-211`:
    ```rust
...
#### Description
A critical design requirement of the streaming snowball is the immutability of the audit trails (append-only timeline). The application relies on BTRFS read-only subvolumes (`-r`) to guarantee this immutability. However, the presence of standard directory fallbacks bypasses these security properties.

#### Code Locations
*   `crates/op-snowball/src/snowball.rs:126-131`
*   `crates/op-snowball/src/snowball.rs:207-211`
