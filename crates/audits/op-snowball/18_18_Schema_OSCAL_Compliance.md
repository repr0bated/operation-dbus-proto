...
| :--- | :--- | :--- | :--- | :--- |
| `BlockEvent` | Struct | `crates/op-snowball/src/footprint.rs:10` | No | Data contract represents an authoritative snowball ledger record using unstructured, unversioned Rust structs. Uses dynamic `simd_json::OwnedValue` for payload. |
| `BlockEvent` | Struct | `crates/op-snowball/src/streaming_snowball.rs:24` | No | Duplicate representation of ledger blocks. Violates Don't Repeat Yourself (DRY) and schema-as-code patterns. |
| `PluginFootprint` | Struct | `crates/op-snowball/src/footprint.rs:52` | No | Hand-rolled struct mapping system audit metrics. Employs untyped dynamic JSON and vector embeddings without proto safety. |
| `PluginFootprint` | Struct | `crates/op-snowball/src/plugin_footprint.rs:11` | No | Secondary implementation of system state footprints with hand-rolled SHA-256 hash formatting. No versioned Protobuf representation. |
| `RetentionPolicy` | Struct | `crates/op-snowball/src/retention.rs:10` | No | Policy schema mapped purely via Serde deserialize. Gaps in cross-language policy mapping. |
| `SnapshotInterval` | Enum | `crates/op-snowball/src/snapshot.rs:9` | No | Local enumeration parsing unversioned env variables without standardized configuration schemas. |
...
| :--- | :--- | :--- | :--- |
| **AU-10 Non-repudiation**<br>*(Cryptographic Ledger Audit Trail)* | `crates/op-snowball/src/snowball.rs:141`<br>`crates/op-snowball/src/streaming_snowball.rs:252` | Missing | Timing blocks are written as SHA-256 validated trails to BTRFS subvolumes, but no OSCAL Component Definition maps this capability to formal AU-10 or AU-12 controls. |
| **CP-9 Information System Backup**<br>*(Local Subvolume Snapshots)* | `crates/op-snowball/src/snowball.rs:164`<br>`crates/op-snowball/src/streaming_snowball.rs:320` | Missing | Disaster recovery subvolumes are snapshotted automatically. Lack of OSCAL System Security Plan (SSP) mapping prevents compliance verification of state retention. |
| **CP-9 Information System Backup**<br>*(Remote Ledger Replication)* | `crates/op-snowball/src/snowball.rs:222`<br>`crates/op-snowball/src/streaming_snowball.rs:360`<br>`crates/op-snowball/src/streaming_snowball.rs:381` | Missing | Remote streaming of timing/vector state using `btrfs send` to external endpoints is not registered as a CP-9/CP-10 transmission mechanism in machine-readable OSCAL controls. |
...
* **Location:** 
  * `crates/op-snowball/src/snowball.rs:228`
  * `crates/op-snowball/src/streaming_snowball.rs:369`
  * `crates/op-snowball/src/streaming_snowball.rs:400`
...
1. **Resolve Command Injection (Critical):**
   ...
   * Rewrite the replication command to execute separate child processes using safe argument vectors. Pipe standard output of the first command directly to standard input of the second:
   ...
   let ssh_child = tokio::process::Command::new("ssh")
       .arg(remote)
       .args(["btrfs", "receive", "/var/lib/snowball/vectors/"])
...
2. **Transition to Protocol Buffers (Schema-as-Code):**
   * Define `BlockEvent` and `PluginFootprint` in a versioned `.proto` file (e.g., `op_snowball/v1/ledger.proto`).
...
4. **Incorporate OSCAL Compliance Artifacts:**
   * Create an OSCAL component definition file (`compliance/component-definition.json`) mapping `op-snowball` to NIST SP 800-53 security controls.
   * Map `btrfs_numa_integration.rs` and `snowball.rs` to **AU-10 (Non-repudiation)** and **CP-9 (Information System Backup)**. Reference the cryptographic hashing and BTRFS subvolume snapshot replication implementations directly.