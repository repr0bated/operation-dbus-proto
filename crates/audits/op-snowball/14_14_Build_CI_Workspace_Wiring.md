# OP-Snowball Production Security and Quality Audit

## 1. Build Audit & Workspace Analysis

### Workspace Configuration & Crate Inheritance
* **Edition**: The workspace uses the `2021` edition, defined globally in the root `Cargo.toml`. The `op-snowball` crate inherits this via `edition.workspace = true` in `crates/op-snowball/Cargo.toml`.
* **Rust Version**: No `rust-version` field is specified in either the workspace `Cargo.toml` or `crates/op-snowball/Cargo.toml`.
* **Crate Type & Entry Points**: `op-snowball` is configured purely as a library crate. There are no standalone binary (`[[bin]]`) targets or examples declared in `crates/op-snowball/Cargo.toml`.
* **Workspace Dependency Management vs. Local Overrides**:
  ...
  * `op-snowball` defines a local path override for its sister crate `op-cache`: `op-cache = { path = "../op-cache" }`. This bypasses workspace dependency versioning for internal integration, which is acceptable for co-located workspace crates but can lead to synchronization issues if crates are published independently.

### Codegen Risk Assessment (`build.rs`)
* No `build.rs` file is checked in or provided in the source files for the `op-snowball` crate. Consequently, there are no immediate local codegen build-time risks within `op-snowball` itself.
...
* **Invocations**: No `.proto` compiler invocations occur in `crates/op-snowball`. However, workspace metadata shows that other crates (e.g., `op-chat` and `op-services`) invoke `prost-build` or `tonic-build` to generate Rust gRPC/Protocol Buffer structs.
...
* **Schema-As-Code Violations**: 
  * In `crates/op-snowball/src/footprint.rs:11` and `crates/op-snowball/src/footprint.rs:43`, the system defines major data contracts (`BlockEvent` and `PluginFootprint`) as local ad-hoc Rust structs.
  * In `crates/op-snowball/src/footprint.rs:49`, `PluginFootprint` specifies: `pub metadata: HashMap<String, simd_json::OwnedValue>`. This represents a design pattern where data contracts are expressed as ad-hoc, unstructured JSON metadata maps rather than strictly versioned, typed schemas.
...
* **Citations**: 
  * `crates/op-snowball/src/snowball.rs:256-263`
  * `crates/op-snowball/src/streaming_snowball.rs:474-480`
  * `crates/op-snowball/src/streaming_snowball.rs:514-517`
* **Vulnerability Analysis**:
  * In `crates/op-snowball/src/snowball.rs:256-263`, `stream_to_remote` formats the parameter `remote_path` directly into a shell command executed via `sh -c`:
    ```rust
    ...
    ```
  * In `crates/op-snowball/src/streaming_snowball.rs:474-480`, `stream_vectors` similarly formats the parameter `remote` into a shell string executed via `bash -c`.
  * In `crates/op-snowball/src/streaming_snowball.rs:514-517`, `stream_to_replicas` formats process-substitution syntax strings using elements of the `replicas` slice and executes them directly in `bash -c`.
...
* **Citations**: 
  * `crates/op-snowball/src/btrfs_numa_integration.rs:148-149`
  * `crates/op-snowball/src/snowball.rs:216-217`
  * `crates/op-snowball/src/streaming_snowball.rs:317-318`
* **Vulnerability Analysis**:
...
* **Citations**: 
  ...
  * `crates/op-snowball/src/snowball.rs:215`
  * `crates/op-snowball/src/btrfs_numa_integration.rs:138-141`
* **Vulnerability Analysis**:
  * In `btrfs_numa_integration.rs:138-141`, `get_cached_block` constructs a file path by appending `block_hash` to a directories structure:
    ...
    ```
  * In `snowball.rs:207` (`write_state`) and line `215` (`read_state`), file paths are constructed directly from a `key` parameter:
    ```rust
...
* **Citations**: 
  * `crates/op-snowball/src/snowball.rs:104-114`
  * `crates/op-snowball/src/snowball.rs:182-192`
* **Vulnerability Analysis**:
...
* **Citations**: 
  * `crates/op-snowball/src/btrfs_numa_integration.rs:159-179`
* **Vulnerability Analysis**:
