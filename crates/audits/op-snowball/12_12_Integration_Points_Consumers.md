# Workspace Integration & Control Plane Audit: op-snowball

## 1. Workspace Integration Analysis

### Crates Depending on `op-snowball`
Based on the workspace `Cargo.toml` and `Cargo.lock`, the following internal crates explicitly depend on `op-snowball`:
...
No D-Bus service names or object paths are registered within the provided files of `op-snowball`. The crate serves strictly as a library providing streaming snowball storage, subvolume management, and NUMA-aware caching.

### HTTP / gRPC Endpoints
No HTTP or gRPC endpoints are exposed by the `op-snowball` crate in the provided files.

### Circular Dependency Evaluation
`op-snowball` depends on `op-cache` and `op-core`. Downstream crates (`op-dbus`, `op-introspection`, `op-plugins`, `op-projection`, and `op-state`) depend on `op-snowball`. There is no evidence in the provided manifests of direct circular dependencies (e.g., `op-cache` or `op-core` depending back on `op-snowball`), maintaining a clean acyclic directed graph (`op-core` $\rightarrow$ `op-cache` -> `op-snowball` $\rightarrow$ downstream consumers).
...
*   **`crates/op-snowball/src/btrfs_numa_integration.rs:105-115`**: Ad-hoc JSON footprint serialization contract constructed dynamically using the `simd_json::json!` macro rather than a versioned schema struct.
*   **`crates/op-snowball/src/btrfs_numa_integration.rs:147-172`**: Manual, ad-hoc key traversal of the JSON AST (`block_data["plugin_id"]`, `block_data["operation"]`, etc.) to reconstruct a `PluginFootprint` rather than utilizing a schema-backed deserializer.
*   **`crates/op-snowball/src/footprint.rs:112-117`**: Dynamic payload generation mapping metadata and hashes to an unversioned, unstructured JSON object via `simd_json::json!`.
*   **`crates/op-snowball/src/streaming_snowball.rs:270-305`**: Ad-hoc creation of timing and vector metadata payloads, leading to potential structural drifts between components accessing the block data.
...
*   **Citations**: 
    *   `crates/op-snowball/src/snowball.rs:258-268`
    *   `crates/op-snowball/src/streaming_snowball.rs:442-459`
    *   `crates/op-snowball/src/streaming_snowball.rs:475-498`
...
*   **Citations**:
    ...
    *   `crates/op-snowball/src/snowball.rs:231`
    *   `crates/op-snowball/src/streaming_snowball.rs:348`
...
*   **Citations**:
    ...
    *   `crates/op-snowball/src/streaming_snowball.rs:334-335`
...
*   **Citations**:
    *   `crates/op-snowball/src/snowball.rs:434-451`
...
*   **Citations**:
    *   `crates/op-snowball/src/plugin_footprint.rs:115-121`
    *   `crates/op-snowball/src/plugin_footprint.rs:133`
    *   `crates/op-snowball/src/lib.rs:1-25`
...
*   **Remediation**: Add a conditionally compiled module declaration `#[cfg(feature = "ml")] pub mod ml;` to `crates/op-snowball/src/lib.rs` and verify its integration with the workspace-level machine learning implementations.