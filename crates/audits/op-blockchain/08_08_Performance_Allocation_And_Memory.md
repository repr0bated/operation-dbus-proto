# Production Security & Quality Audit: op-blockchain Crate
...
*   **File**: `crates/op-blockchain/src/blockchain.rs:248`
*   **File**: `crates/op-blockchain/src/streaming_blockchain.rs:362`
*   **File**: `crates/op-blockchain/src/streaming_blockchain.rs:395`
*   **Exploitability**: **Directly Exploitable**. The system constructs shell commands using unescaped string formatting and passes them directly to `sh -c` or `bash -c`. 
    *   In `blockchain.rs:248`, the `remote_path` parameter is formatted directly into a shell string:
        ```rust
        ...
        ```
    *   In `streaming_blockchain.rs:362`, the `remote` parameter is injected into a `bash -c` block:
        ```rust
        ...
        ```
    *   In `streaming_blockchain.rs:395`, the `replicas` string slice elements are directly mapped into `tee` process substitutions inside a shell context:
        ```rust
...
*   **File**: `crates/op-blockchain/src/btrfs_numa_integration.rs:163`
*   **File**: `crates/op-blockchain/src/blockchain.rs:324`
*   **File**: `crates/op-blockchain/src/streaming_blockchain.rs:309`
...
*   **File**: `crates/op-blockchain/src/footprint.rs:8`
*   **File**: `crates/op-blockchain/src/footprint.rs:46`
*   **File**: `crates/op-blockchain/src/plugin_footprint.rs:11`
*   **Vulnerability**: The blockchain's core data contracts (`BlockEvent` and `PluginFootprint`) are defined purely as ad-hoc, unversioned Rust structs with standard Serde serializers. There is no machine-readable, language-independent source of truth (such as Protocol Buffers `.proto` files or OSCAL compliance schemas) to dictate the structure of these blocks. 
    Because blockchain blocks are stored persistently on disk, any future changes to these structures will break backward compatibility. Unversioned structs parsed in production will lead to silent deserialization errors, broken state machines, or crash-loops when reading older blocks.
...
*   **File**: `crates/op-blockchain/src/btrfs_numa_integration.rs:189`
...
*   **File**: `crates/op-blockchain/src/streaming_blockchain.rs:245`
*   **File**: `crates/op-blockchain/src/streaming_blockchain.rs:259`
...
*   **File**: `crates/op-blockchain/src/btrfs_numa_integration.rs:157`
*   **File**: `crates/op-blockchain/src/blockchain.rs:231`
*   **File**: `crates/op-blockchain/src/blockchain.rs:241`
*   **File**: `crates/op-blockchain/src/streaming_blockchain.rs:569`
...
*   **File**: `crates/op-blockchain/src/plugin_footprint.rs:125`
*   **File**: `crates/op-blockchain/src/footprint.rs:24`
*   **Vulnerability**:
...
*   **File**: `crates/op-blockchain/src/footprint.rs:24`
*   **File**: `crates/op-blockchain/src/plugin_footprint.rs:34`
*   **File**: `crates/op-blockchain/src/plugin_footprint.rs:77`
...
The `op-blockchain` crate does not explicitly instantiate any direct memory maps (`memmap2`, `mmap`, `MmapMut`) or directly invoke `sled` within the provided files. Sled is defined as a workspace-level dependency in the root `Cargo.toml`, but is not utilized inside the audited scope.
