...
async fn main() -> anyhow::Result<()> {
    let email = std::env::args().nth(1).expect("usage: check_and_delete_user <email>");
    let store = UserStore::new("/var/lib/op-dbus/users-cozo").await?;
    let user = store.get_user_by_email(&email).await;
    match user {
        Some(u) => {
            println!("Found user: id={} email={} verified={} container={:?} route_id={:?} connected={}",
                u.id, u.email, u.email_verified, u.privacy_container_name, u.privacy_route_id, u.privacy_network_connected);
