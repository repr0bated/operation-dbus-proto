...
import { GrpcPageRenderer } from "@/components/json/GrpcPageRenderer";
import { eventChainService } from "@/grpc/client";
import { structToObject } from "@/grpc/google/protobuf/struct";
import { useEventStore } from "@/stores/event-store";
import { cn } from "@/lib/utils";
import { Send, Loader2, AlertTriangle, ShieldCheck, Database, RefreshCcw } from "lucide-react";
...
const COMPLIANCE_STYLE: Record<ComplianceStatus, { text: string; dot: "ok" | "warn" | "error" | "offline" }> = {
...
function toComplianceStatus(payload: Record<string, unknown>): ComplianceStatus {
  if (payload.piiFlagged === true || payload.pii_flagged === true) {
    return "warn";
  }

  const outcome = getString(payload, "outcomeClass") || getString(payload, "outcome_class");
...
function toTraceEntry(match: { pointId: string; score: number; payload?: Record<string, unknown> }): QdrantTraceEntry {
  ...
  const decision = getString(payload, "decisionOutput") || getString(payload, "decision_output");
  const source =
...
export const AccountabilityPage: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { id: "sys-1", role: "system", content: "Snowball session established. Awaiting agent interaction.", timestamp: Date.now() },
  ]);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      setSending(false);
      setActiveSled((prev) => prev ? { ...prev, mutationIndex: prev.mutationIndex + 1 } : prev);
    }, 600);
  };
  ...
  const handleTraceSearch = useCallback(async () => {
    ...
    try {
      const response = await eventChainService.searchSemanticTrace({ limit: DEFAULT_TRACE_LIMIT });
      setActiveSled((prev) => ({
      ...
      setTraceError(error instanceof Error ? error.message : "Semantic trace request failed");
      setTraceResults([]);
    } finally {
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between gap-4 px-4 py-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Two-pane confrontation: Agent Dialog above, Qdrant Semantic Trace below.
            </p>
          </div>
            ...
            {activeSled ? (
              <Pill variant="default">
                <span className="font-mono text-[11px] truncate max-w-[200px]">
                  Trace: {activeSled.traceId || "—"} | Idx: {activeSled.mutationIndex}
                </span>
              </Pill>
            ) : (
              <Pill variant="warn">
      ...
      <div className="flex flex-col flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-h-0 border-b border-border">
          ...
          <div ref={threadRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-3" role="log">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={cn(
              ...
              >
                {msg.role !== "user" && (
                  <div className={cn(
                    ...
                    msg.role === "system" ? "bg-muted text-muted-foreground" : "bg-primary/20 text-primary",
                  )}>
            ...
            {sending && (
              <div className="flex gap-3">
                <div className="h-7 w-7 rounded-full bg-primary/20 flex items-center justify-center text-[10px] font-bold text-primary shrink-0">AI</div>
          ...
          <div className="border-t border-border px-4 py-2.5 shrink-0 bg-background">
            ...
            <div className="flex gap-2">
              <textarea
                ...
                disabled={!connected}
                placeholder={connected ? "Confront or command the agent… (↩ send)" : "Stream offline…"}
        ...
        {/* MIDDLE PANE: gRPC Page Elements — PluginSchema Direct Read */}
        <div className="flex-[0_0_auto] max-h-[35%] flex flex-col min-h-0 border-b border-border">
          <div className="px-4 py-1.5 border-b border-border bg-[hsl(var(--bg-elevated))] flex items-center justify-between">
            ...
            <span className="text-[10px] text-muted-foreground font-mono">
              PluginSchema · 1:1 Direct Read · SchemaEngine /dev/shm
            </span>
          </div>
          <div className="flex-1 overflow-auto">
            <GrpcPageRenderer
              streamOptions={{
                pluginIds: [],
                tags: ["accountability", "compliance"],
                includeInitialState: true,
              }}
              readOnly={false}
              actorId={`ui:accountability:${activeSled?.traceId || "anonymous"}`}
              onMutate={(pluginId, member, value) => {
                setActiveSled((prev) => prev ? { ...prev, mutationIndex: prev.mutationIndex + 1 } : prev);
                void handleTraceSearch();
              }}
            />
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="px-4 py-2 border-b border-border flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-sm text-foreground">Schema-driven semantic retrieval</div>
              <div className="text-xs text-muted-foreground font-mono truncate">
                Trace {activeSled?.traceId || "unavailable"} · Limit {DEFAULT_TRACE_LIMIT}
              </div>
            </div>
            <button
            ...
            >
              {traceLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCcw className="h-3.5 w-3.5" />}
              Refresh
            </button>
          </div>
            ...
            <div className="px-4 py-3 space-y-2">
              ...
              {!traceLoading && !traceError && traceResults.length === 0 && (
                <div className="flex flex-col items-center justify-center py-8 text-sm text-muted-foreground gap-2">
                  ...
                  <span>No semantic trace matches yet. Refresh after the shuttle indexes this session.</span>
                </div>
              )}

              {!traceLoading && traceResults.map((entry) => {
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        <span className={cn("font-medium uppercase", style.text)}>
                          {entry.complianceStatus}
                        </span>
                        <span className="text-muted-foreground">·</span>
                        <span className="font-mono text-muted-foreground">{entry.outcomeClass}</span>
                        <span className="text-muted-foreground">·</span>
                        <span className="text-muted-foreground">score {entry.score.toFixed(2)}</span>
                      </div>
                      <span className="text-[10px] font-mono text-muted-foreground">
                        {entry.source}
                      </span>
                    </div>
                    <div className="text-sm text-foreground leading-relaxed">{entry.summary}</div>
                    {entry.decision && (
                      <div className="text-xs text-muted-foreground">{entry.decision}</div>
                    )}
                    ...
                    <details className="rounded-md border border-border/60 bg-background/40">
                      ...
                      <div className="px-3 pb-3">
                        <JsonRenderer data={entry.payload} defaultMode="tree" maxDepth={4} />
                      </div>
                    </details>
                  </div>
                );
