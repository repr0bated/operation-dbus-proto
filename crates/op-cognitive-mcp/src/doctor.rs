...
pub async fn run_diagnostics(
    ...
    match memory_store.get_stats().await {
        ...
        Err(e) => {
            ...
            recommendations.push("Memory store is unreachable. Check SQLite database path and permissions.".into());
        }
    }
    ...
    if remaining == 0 {
        recommendations.push("Query quota exhausted. Consider upgrading tier or waiting for daily reset.".into());
    }
    components.push(ComponentStatus {
    ...
    components.push(ComponentStatus {
        ...
        status: if gemini_available { "ok" } else { "unavailable" }.into(),
        details: serde_json::json!({
    ...
    // 6. Auth Status
    let auth_method = std::env::var("COGNITIVE_MCP_AUTH_METHOD")
        .unwrap_or_else(|_| "chrome_profile".into());
    components.push(ComponentStatus {
        name: "auth".into(),
...
mod tests {
    ...
    fn should_get_query_history_with_turns() {
        ...
        mgr.append_turn("conv-1", crate::session::QueryTurn {
