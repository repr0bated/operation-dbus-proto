...
/// Outcome of one [`StreamingSnowball::replicate`] pass.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl StreamingSnowball {
    ...
    pub async fn new_with_interval(
        ...
        info!(
            "Streaming snowball initialized at {:?} with {} interval, resuming after block {}",
...
mod tests {
    ...
    fn snapshot_names_are_aligned_per_subvolume() {
        assert_eq!(
            StreamingSnowball::snapshot_name("timing", 7),
            "SNP-timing-000007"
        );
        assert_eq!(
            StreamingSnowball::snapshot_name("vectors", 7),
            "SNP-vectors-000007"
        );
        assert_eq!(
            StreamingSnowball::snapshot_name("state", 7),
            "SNP-state-000007",
