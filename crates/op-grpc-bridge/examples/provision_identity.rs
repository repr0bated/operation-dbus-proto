...
async fn main() -> anyhow::Result<()> {
    let pubkey = std::env::args().nth(1).expect("usage: provision_identity <wireguard_pubkey> [psk_b64] [ttl_seconds]");
    ...
    let client = RemoteOperationClient::new(pool, "http://127.0.0.1:8090", "provision-identity-tool");
