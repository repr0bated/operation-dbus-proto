...
Phase 1 was partially live at the historical checkpoint, but it is not live
after the network outage. The operator deliberately deleted the OVS bridge to
regain connectivity, restoring direct `eth0` as the emergency fallback. OVSDB
therefore has no `ovsbr0`; `svc0`/`pub0` are absent; and xray and NetMaker have
only loopback. This is a known protective state, not unexplained OVS drift.
Native OVS/rtnetlink attachment and basic L3
transit described below are historical evidence plus a target, not current
state. The design separates:
...
The original live-state checkpoint and the post-outage recheck are both from
2026-08-03. The post-outage recheck is authoritative for activation safety.

### 1.1 Post-outage recovery boundary

Source work may continue, but live recovery is a distinct operation:

1. Preserve and capture the direct-`eth0` fallback that currently provides
   operator access. Do not undo the deliberate bridge deletion remotely.
2. Review a base OVS recovery and exact console rollback before invoking any
   OVS/uplink service. Network-critical services are not auto-restarted.
3. Recreate and independently verify `ovsbr0`, `pub0`, `svc0`, host reachability,
   and service readiness before restoring xray/NetMaker attachments.
4. Keep all new `xray-attachment` and `netmk-*` services disabled until the
   activation defects in §6.5 are corrected and tested on disposable state.
5. Re-run every historical baseline read; no pre-outage checkbox or counter is
   accepted as proof of current state.

---

## 2 · Historical Observed Interim Architecture
...
Additional facts observed before the outage:
...
PID it restores `grpc0`, addresses, and default route, starts/verifies
`xray.service`, and publishes attachment readiness. This preserves the
pre-existing xray application independently of the optional Netmaker feature.

After that stamp, `netmk-egress-policy` generates the reviewed xray projection,
starts/verifies `op-netmk-xray-policy.service`, and only then permits downstream
Netmaker readiness. Do not make `xray.service` require the Netmaker feature
policy: that creates a projection/attachment cycle and lets an optional transit
policy take down existing xray ingress. Disable independent xray boot start if
the host attachment owner is authoritative. Event-stream reconnect uses
bounded backoff and readiness keyed by PID; it is not a polling loop. Neither
unit may modify/reload xray application configuration.
...
Incus xray lifecycle event
       └──► restore grpc0/default ─► StartUnit(xray) ─► attachment stamp

netmk-egress-policy
       └──► render reviewed projection ─► StartUnit(xray policy)
...
### 6.5 Source-draft activation defects found during re-baseline

The current untracked/local draft compiles, but it must not be enabled yet:

1. `xray-attachment/run` calls `StartUnit` for
   `op-netmk-xray-policy.service`, while that unit requires
   `/var/lib/opdbus-runtime/netmk/xray-filter.rules`. The projection is only
   generated by downstream `netmk-egress-policy`, which waits for the
   `xray-attachment` ready stamp. This is a dependency cycle. Split attachment
   readiness from policy start: attachment restores `grpc0`, starts and
   verifies the existing `xray.service`, then publishes PID/interface/service
   readiness. The downstream policy owner generates the projection and only
   then starts/verifies the optional policy unit.
2. `op-netmk-xray-policy` flushes/rebuilds the feature chain and appends two
   `FORWARD` jumps before it rejects stale bypass references or feature-subnet
   NAT. On rejection it leaves the mutations in place. Stage the complete
   target rules, capture the complete prior policy, perform one bounded
   replacement, and restore prior state on any verification failure.
3. `netmk-egress-policy/run` applies host mangle/filter/NAT state before the
   xray policy restart and has no trap that restores the captured bundle if a
   downstream step fails. Its pre-state capture is evidence, not an automatic
   rollback. Implement and test bounded rollback before activation.
4. `netmk-of-restrict/run` mutates controller configuration with
   `ovs-vsctl set-controller`. Move that mutation behind the native
   OVSDB/projected method surface; read-only ofport/controller diagnostics may
   remain independent observations.
5. Runit `run`/`pause` status for the base OVS address/uplink services remained
   green after their owned links disappeared. Add real `check` programs or
   long-running drift verification before those services can count as
   dependencies.
...
source is `deploy/security/capability-grants.json`; its repo, installed, and SHM
copies matched on 2026-08-03. The running bridge and sealed catalog still
predate the MTU method draft, so the grant is necessary but not deployment
evidence.
...
| Source-preserving probe fails | Restore complete captured xray `FORWARD` policy and exact interim SNAT; remove only newly owned state. |
...
| §4 source `10.200.1.1` host chains | Correct target; zero host counters today are explained by xray SNAT plus orphaned host jumps. Xray bypass references are stale/nonmatching and must be replaced. |
