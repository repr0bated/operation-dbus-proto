...
**Transport & identity (zero-trust) — intent vs current live state.** The intended/documented model (see `unix_socket.rs`'s `SHARED_CONTAINER_SOCKET`) is: gRPC (tonic, TLS mandatory) over one shared Unix domain socket per container (`/run/ghostbridge/container.sock`, bind-mounted into the container as a disk device), with the tonic-web bridge demuxing by gRPC service/method via reflection — no per-service TCP ports, no NIC. Identity = WireGuard pubkey, but **WireGuard terminates only at the Oracle decoy — never on this host** (no `wg show` anywhere in the host path; `wgcf*` are WARP egress). The decoy's `op-decoy-issuer` signs a short-lived OIA1 assertion that rides *inner* as gRPC metadata (`x-oracle-identity-assertion-bin`); xray is SNI passthrough and injects nothing; `op-grpc-bridge` is the sole validator. Keys live in Cozo (`human_principal`, `identity_sled`). Derived ids are blake3, not Argon2: `session_id = derive_key("op-identity session-id v1", pubkey)` (= the identity container's name), `principal_id = derive_key("op-identity human-principal v1", pubkey)`, `genesis = mint_genesis(pubkey, chain_head, head_ts, catalog_hash, arrival_ts)` minted once per session. Local service clients (the singleton control-plane chatbot) present the MutationEngine-sealed SID1 envelope (`x-opdbus-sealed-id-bin`) from `/dev/shm/opdbus/credentials/`. `X-Ghostbridge-Footprint`, Argon2(PSK) sessionids, and host `wg-*` tunnels are retired — see `.kiro/specs/control-plane-chatbot-identity/requirements.md`. The control-plane chatbot is a *service* principal and must never be the fallback identity for humans, op-web, or pairing. SESSION bus = the identity-gated plugin surface; SYSTEM bus = local agents/mirror.
