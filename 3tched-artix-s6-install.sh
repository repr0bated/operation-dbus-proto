...
build_project() {
    ...
    # op-web UI (vite) is intentionally not built here — install/build ui/dist
    # yourself later. Release cargo of op-web panics without ui/dist, so skip
    # -p op-web until that exists.
    local cargo_args=()
    if [[ -d ${REPO_DIR}/crates/op-web/ui/dist ]]; then
        cargo_args+=(-p op-web)
    else
        warn "Skipping -p op-web (no crates/op-web/ui/dist — build the vite UI later, then rebuild op-web)"
    fi
    cargo_args+=(
        -p op-projection
        -p op-grpc-bridge
