...
//! - Memory refresh capability for dynamic monitoring
...
impl NumaNode {
    ...
    /// Get memory free in MB
    pub fn memory_free_mb(&self) -> u64 {
        self.memory_free_kb / 1024
    }
...
impl NumaTopology {
    ...
    pub fn detect() -> Result<Self> {
        ...
        if !sys_node_path.exists() {
            tracing::info!("NUMA not available on this system (no /sys/devices/system/node)");
            ...
            if let Some(node_suffix) = name_str.strip_prefix("node") {
                if let Ok(node_id) = node_suffix.parse::<u32>() {
                    ...
                    match Self::parse_node(&node_path, node_id) {
                        ...
                        Err(e) => {
                            tracing::warn!("Failed to parse NUMA node {}: {}", node_id, e);
                        }
                    }
                }
            }
        ...
        if nodes.is_empty() {
            tracing::warn!("No online NUMA nodes found, falling back to single-node");
            return Self::create_single_node_fallback();
        }
            ...
            if part.contains('-') {
                ...
                if range_parts.len() == 2 {
                    let start: u32 = range_parts[0].parse()
                        .map_err(|_| OpDbusError::ConfigError(format!("Invalid CPU range: {}", part)))?;
                ...
                let cpu: u32 = part.parse()
                    .map_err(|_| OpDbusError::ConfigError(format!("Invalid CPU: {}", part)))?;
                cpus.push(cpu);
            }
        }
    ...
    fn detect_current_node() -> Option<u32> {
        ...
        if parts.len() > 38 {
            ...
            if let Ok(node_links) = fs::read_dir(sys_cpu_path) {
    ...
    fn create_single_node_fallback() -> Result<Self> {
        ...
        let (total_kb, free_kb) = Self::read_system_memory()
            .unwrap_or((8 * 1024 * 1024, 4 * 1024 * 1024));
    ...
    pub fn total_memory_mb(&self) -> u64 {
        self.nodes.values().map(|n| n.memory_mb()).sum()
    }
}
...
// ============================================================================
// NUMA OPTIMIZER (Compatibility wrapper)
// ============================================================================
...
impl NumaOptimizer {
    ...
    pub fn new(config: NumaConfig) -> Self {
        let topology = match NumaTopology::detect() {
            Ok(t) => {
                tracing::info!("NUMA topology detected: {} nodes", t.node_count());
                Some(t)
            }
            Err(e) => {
                tracing::info!("NUMA not available: {}", e);
                None
            }
        };
    ...
    pub fn get_optimal_node(&mut self) -> Option<u32> {
        ...
        match self.config.strategy {
            ...
            NumaStrategy::LocalNode => {
                self.config.node_preference.or(Some(topology.optimal_node()))
            }
    ...
    /// Set CPU affinity for current thread (not implemented - logs only)
        ...
        if let Some(ref topology) = self.topology {
            if let Some(node) = topology.get_node(node_id) {
                if !node.cpu_list.is_empty() {
                    tracing::debug!("CPU affinity would be set to node {} CPUs: {:?}", node_id, node.cpu_list);
                }
            }
        }
        Ok(())
...
mod tests {
    ...
    #[test]
    fn test_numa_node_utilization() {
        let node = NumaNode {
            node_id: 0,
            cpu_list: vec![0, 1, 2, 3],
            memory_total_kb: 16 * 1024 * 1024, // 16GB
            memory_free_kb: 8 * 1024 * 1024,   // 8GB
            distance_to_nodes: HashMap::new(),
        };

        assert_eq!(node.cpu_count(), 4);
        assert_eq!(node.memory_mb(), 16 * 1024);
