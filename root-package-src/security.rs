...
// Global security validator instance (initialized lazily)
...
pub fn init_security_validator() {
    let _ = SECURITY_VALIDATOR.set(Arc::new(SecurityValidator::with_admin_profile()));
}

/// Get the global security validator
pub fn get_security_validator() -> Option<Arc<SecurityValidator>> {
    SECURITY_VALIDATOR.get().cloned()
}

/// Ensure validator is initialized and return it
pub fn ensure_security_validator() -> Arc<SecurityValidator> {
    init_security_validator();
    get_security_validator().expect("Security validator should be initialized")
}
...
mod tests {
    ...
    async fn test_custom_profile() {
        let validator = SecurityValidator::new(
            ToolSecurityProfile::custom("network-admin", vec!["ip", "ping", "traceroute"])
        );

        assert!(validator.check_command("ip addr").await.is_ok());
        assert!(validator.check_command("ping localhost").await.is_ok());
        assert!(validator.check_command("rm -rf /").await.is_err());
    }
}
