...
4. Source the profile helper before running Zenflow:
   ...
   The script will fetch an access token with `gcloud auth print-access-token`, set the necessary `OPENCLAW_*` environment variables, and print the active profile for verification.
...
- `OPENCLAW_TOKEN` is the OpenClaw gateway bearer token. ADC is still used by the target OpenClaw agent for Gemini/Vertex auth, but the token itself is not derived from ADC.
...
4. Once the trial credit is used or you switch models, unset the profile:
   ```bash
   unset ZENFLOW_PROFILE OPENCLAW_TOKEN OPENCLAW_MODEL
   ```
