...
export function ChatbotLayout({
  ...
  // Auto-send initial greeting on first load
  const initializedRef = useRef(false);
  useMemo(() => {
    if (!initializedRef.current && !sending && messages.length === 0 && systemPrompt) {
      initializedRef.current = true;
      // Trigger initial message by simulating a form submission with a greeting
      setTimeout(() => {
        const event = new Event("submit", { bubbles: true, cancelable: true }) as any;
        // We'll set input to a greeting and trigger submit
        // This is a bit hacky but ensures the system prompt gets sent
      }, 100);
    }
