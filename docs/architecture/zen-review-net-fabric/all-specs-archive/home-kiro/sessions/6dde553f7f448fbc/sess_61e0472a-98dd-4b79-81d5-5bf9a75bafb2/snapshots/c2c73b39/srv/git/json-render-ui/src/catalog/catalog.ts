  ...
  components: {
    ...
    antigravityGroupChat: {
      props: z.object({
        ...
        title: z.string().nullable(),
        botSide: z.enum(["left", "right"]).nullable(),
        leftLabel: z.string().nullable(),
        rightLabel: z.string().nullable(),
      }),
      description:
        "User vs Chatbot group chat. One side is designated as the chatbot, user selects which side they speak as. Each side has its own model selector.",
    },
    catalogGallery: {
      props: z.object({
        className: z.string().nullable(),
