...
export function PluginChatView({
  ...
  const handleSubmitMessage = async (messageText: string) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: assistantId, role: 'assistant', content: '', spec: null, text: '' },
    ]);
    ...
    abortRef.current = streamChat(
      ...
      (chunk) => {
        fullContent += chunk;
        // Parse content to extract spec and text
        const { spec, text } = parseContent(fullContent);
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === assistantId
              ? { ...msg, content: fullContent, spec, text }
              : msg
          )
        );
      },
