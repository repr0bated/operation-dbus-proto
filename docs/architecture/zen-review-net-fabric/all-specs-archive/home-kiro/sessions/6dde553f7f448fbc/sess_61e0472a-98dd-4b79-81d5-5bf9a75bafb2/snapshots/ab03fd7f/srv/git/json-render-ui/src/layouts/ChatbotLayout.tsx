...
import { useAntigravityChat, type ChatMode } from "@/stream/use-antigravity-chat";
...
export function ChatbotLayout({
  ...
  const enrichedMessages = useMemo(
    () =>
      messages.map((msg) => {
        ...
        console.log(`[ChatbotLayout] msg role=${msg.role} hasSpec=${!!spec} textLen=${text?.length || 0}`);
  ...
  useMemo(() => {
    const latestWithSpec = [...enrichedMessages].reverse().find((msg) => msg.role === "agent" && msg.spec);
    if (latestWithSpec?.spec) {
      console.log(`[ChatbotLayout] Setting currentSpec from message ${latestWithSpec.id}`);
        ...
        <div className="w-80 flex flex-col bg-neutral-900 border-l border-neutral-800">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3 min-h-0">
            {enrichedMessages.length === 0 ? (
              <div className="text-sm text-neutral-500 space-y-2 py-8">
                <p>💬 Start a conversation</p>
                <p className="text-xs">Try asking: "show me the dashboard" or "draw the navigation"</p>
              </div>
            ) : (
