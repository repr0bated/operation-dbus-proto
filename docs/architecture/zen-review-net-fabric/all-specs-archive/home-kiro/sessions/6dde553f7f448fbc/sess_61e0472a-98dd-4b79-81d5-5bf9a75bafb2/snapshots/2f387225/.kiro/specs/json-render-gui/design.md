...
|------------|-------------|---------------|
| `$state.children` | Full BoundedChildren object | `{ items: [...], total: 42 }` |
| `$state.children.items` | Array for `repeat` binding | `[{id: "...", ...}, ...]` |
| `$state.children.total` | Total count across pages | `42` |
| `$state.children.next_cursor` | Pagination cursor (truthy = more) | `"eyJ..."` or `null` |
| `item.id` | Current item's unique ID | `"019abc..."` |
| `item.status` | Current item's ChildStatus | `"ok"`, `"pending"`, etc. |
| `item.data` | Plugin-specific payload object | `{ networks_found: 12, ... }` |
| `item.data.{field}` | Nested field access | `12` |
| `item.actions` | Array of action names | `["view", "export", "delete"]` |
| `{ "$entries": "item.data" }` | Object → `[{key, value}]` array | For iterating object fields |
| `{ "$template": "..." }` | String interpolation | `"12 of 42"` |
| `{ "$switch": {...} }` | Conditional value | Status → color mapping |
