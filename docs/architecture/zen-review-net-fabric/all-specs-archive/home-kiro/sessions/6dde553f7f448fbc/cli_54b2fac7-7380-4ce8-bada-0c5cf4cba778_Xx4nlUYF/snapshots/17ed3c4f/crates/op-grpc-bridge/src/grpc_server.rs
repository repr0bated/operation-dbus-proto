        ...
        let stream = stream! {
            ...
            loop {
                tokio::select! {
                    ...
                    _ = keepalive.tick() => {
                        // Send a heartbeat: empty StateChange with change_id="keepalive"
                        // to prevent browser idle-connection timeouts.
                        yield Ok(ProtoStateChange {
                            change_id: "keepalive".to_string(),
                            ..Default::default()
                        });
                    }
                }
