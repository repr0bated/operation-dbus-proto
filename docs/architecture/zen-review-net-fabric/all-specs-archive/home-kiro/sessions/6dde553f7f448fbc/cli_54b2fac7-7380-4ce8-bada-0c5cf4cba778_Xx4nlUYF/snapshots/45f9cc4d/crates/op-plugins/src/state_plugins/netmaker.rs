...
pub async fn dispatch_netmaker_method(method: &str, args: &JsonValue) -> Result<JsonValue> {
    ...
    match method {
        "join_network" => {
            ...
            plugin
                .join_network(&input.network, input.token.as_deref().unwrap_or(""))
                .await?;
