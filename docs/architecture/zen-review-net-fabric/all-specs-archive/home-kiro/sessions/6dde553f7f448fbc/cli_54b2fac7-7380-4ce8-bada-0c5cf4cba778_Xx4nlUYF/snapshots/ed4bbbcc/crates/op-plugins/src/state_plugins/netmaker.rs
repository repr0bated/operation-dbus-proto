...
/// Passthrough output for methods that return the live Netmaker server API
/// response verbatim — the shape is Netmaker's, not ours, so it is declared
/// as an open JSON value rather than a fabricated typed struct.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(transparent)]
pub struct NetmakerApiOutput(#[schemars(with = "serde_json::Value")] pub serde_json::Value);
