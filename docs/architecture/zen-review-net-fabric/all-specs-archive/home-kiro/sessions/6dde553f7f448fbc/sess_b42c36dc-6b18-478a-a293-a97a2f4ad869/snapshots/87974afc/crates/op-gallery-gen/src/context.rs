...
pub fn load_from_dir(dir: &Path) -> Result<CatalogLoadResult> {
    ...
    let manifest_bytes = std::fs::read(&manifest_path)
        .with_context(|| format!("Failed to read catalog manifest at {}", manifest_path.display()))?;
    let manifest: serde_json::Value = serde_json::from_slice(&manifest_bytes)
        .context("Failed to parse catalog manifest JSON")?;
    ...
    for plugin_id in &plugin_ids {
        match op_blob::catalog::read_plugin_state_store_schema(dir, plugin_id) {
            Some(plugin_schema) => {
                match convert_plugin_schema(plugin_id, &plugin_schema) {
...
fn convert_plugin_schema(
    ...
    let raw_json = serde_json::to_value(schema)
        .unwrap_or_else(|_| serde_json::json!({ "name": plugin_id, "error": "serialization failed" }));
...
fn convert_constraints(constraints: &[op_state_store::Constraint]) -> Constraints {
    ...
    for c in constraints {
        match c {
            ...
            op_state_store::Constraint::Pattern { regex } => {
                result.pattern = Some(regex.clone())
            }
...
pub fn read_catalog_hash_from_shm() -> Option<String> {
    op_blob::catalog::ActiveReflectionCatalog::read_catalog_hash(
        Path::new(op_blob::catalog::DEFAULT_SHM_DIR),
    )
}
