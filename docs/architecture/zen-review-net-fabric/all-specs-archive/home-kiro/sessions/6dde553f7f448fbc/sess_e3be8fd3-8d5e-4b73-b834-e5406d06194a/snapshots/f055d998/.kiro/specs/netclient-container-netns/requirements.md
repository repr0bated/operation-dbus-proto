...
- A long-running host runit xray-attachment owner SHALL perform initial
  ...
  restores `grpc0`/addresses/default route, starts
  `op-netmk-xray-policy.service`, verifies it, then starts `xray.service`.
  `xray.service` SHALL be disabled from independent boot start or have a
  packaged dependency preventing start before policy. Event-stream reconnect
  uses bounded backoff and PID-keyed readiness, not polling.
