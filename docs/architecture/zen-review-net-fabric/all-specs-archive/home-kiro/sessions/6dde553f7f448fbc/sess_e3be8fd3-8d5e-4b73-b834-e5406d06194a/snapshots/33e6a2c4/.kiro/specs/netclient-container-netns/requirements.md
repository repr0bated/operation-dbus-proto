...
- Presence of scoped grants is not proof that the current direct helper is
  ...
  - `replace_default_route(ReplaceDefaultRouteInput { name, gateway,
    netns_pid }) -> RtnetlinkMutationOutput`, with exact delete/add/read-back
    semantics rather than accepting arbitrary `EEXIST`;
  - authorized `remove_port`/port-delete for deterministic teardown.
...
- `netclient.service` SHALL declare `Requires=`/`After=` on the named Netmaker
  network-ready and adapter units. The host `netmk-netclient-start` runit stage
  waits for its host readiness stamps and then calls
  `org.freedesktop.systemd1.Manager.StartUnit("netclient.service", "replace")`
  over `unix:path=/proc/<fresh-pid>/root/run/dbus/system_bus_socket`.
- Adapter join/leave/restart implementation SHALL use the same systemd D-Bus
  lifecycle and remove the current `sv restart netclient` path. OCI supervision
  metadata SHALL be `systemd` for this deployment.
- `op-netmk-xray-policy.service` SHALL start only after the durable
  xray-attachment reconciler has restored and verified `grpc0`.
...
- Adapter health, Netmaker health, and Netclient process state SHALL be
  independently verified. For one named test peer, record test start time and
  RX/TX counters, generate a bounded peer probe, require a latest handshake
  newer than test start and no more than 180 seconds old, and require RX/TX
  bytes or packets to increase. Repeat after container and host-policy restarts.
