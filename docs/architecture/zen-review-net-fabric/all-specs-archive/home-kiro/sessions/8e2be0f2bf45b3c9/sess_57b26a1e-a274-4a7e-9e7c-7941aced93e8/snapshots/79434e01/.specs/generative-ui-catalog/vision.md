...
- **Store raw and pre-resolved.** Raw keeps one honest source; pre-resolved (refs inlined) is what a stateless model with a three-line prompt actually needs. Raw also lets `validate_spec` check a generated UI against the real schema.
