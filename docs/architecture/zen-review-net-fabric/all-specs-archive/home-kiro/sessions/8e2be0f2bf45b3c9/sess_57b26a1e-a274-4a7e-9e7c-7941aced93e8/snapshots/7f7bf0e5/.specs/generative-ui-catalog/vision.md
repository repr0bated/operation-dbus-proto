...
## 9.1 The state model — desired state *is* the state

> "The desired state is the state. If you don't like it, mutate it."

There is no desired-versus-actual distinction. No drift, no reconciliation loop, no sync. What is declared is what is. Change happens exactly one way: **mutation.**
...
**"State sync" is a category error here.** Syncing presupposes two things that can disagree. With one source of truth there is nothing to sync, and a `GetState` is simply a read of the state — not a snapshot to be compared against something else.

What that removes from the UI, which matters because a designer will otherwise reach for it:

- No drift indicators, no "out of date" badges, no diff view between desired and actual — none of those have a referent.
...
**Useful as a review signal:** if a promoted element contains a drift, sync, staleness, or refresh affordance, that is evidence the brief leaked vocabulary — worth treating as a defect in the prompt, not just in the element.
