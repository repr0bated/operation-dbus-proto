...
pub async fn generate_plugin_page(
    ...
    // Validate spec structure and grammar
    let validator = op_gallery_gen::SpecValidator::with_catalog(catalog_guard);
    let validation = validator.validate(&generated.spec);
    if !validation.valid {
