...
/// Role-to-component mapping table for plugin page generation.
/// This is included in the system message when generating a plugin page spec.
pub const ROLE_COMPONENT_TABLE: &str = r#"
| UiRole | Catalog Component | Notes |
|---|---|---|
| surface | (omit) | becomes route, not element |
| display-value | stateValue | |
| state-flag | statusDot | |
| collection-view | streamObject | with repeat |
| record-view | card | |
| value-list | container | with repeat |
| binary-control | pill | read-only |
| text-control | stateValue | read-only |
| numeric-control | statCard | read-only |
| multi-choice | stateValue | format=raw, read-only |
| editable-collection | streamObject | with repeat, read-only |
| record-editor | card | read-only |
| structured-control | card | read-only |
| validation-carrier | (omit) | |
| hydration-source | (omit) | |
| trigger-binding | (omit) | |
| repeat-binding | (omit) | |
"#;
...
pub struct GenerationContext {
    ...
    /// The catalog's own system prompt, from `schemas/json-render/catalog.prompt.md`.
    ///
    /// Output contract, directives, state model and every available component —
    /// generated by the catalog that the renderer and the admission gate both
    /// use, so a model cannot be taught a component the gate will reject. Never
    /// hand-written: see [`crate::catalog_guard`].
...
/// Context for plugin-scoped generation (single plugin page spec).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TargetPluginContext {
    pub plugin_id: String,
    pub projections: Vec<op_state_store::subid_ui::UiSubidProjection>,
    pub ui_surfaces: UiSurfaceProjection,
}
...
impl GenerationContext {
    ...
    ///
    /// `catalog_prompt` comes from [`crate::CatalogGuard::prompt`], i.e. from the
    /// same artifact the admission gate compiles. It is a parameter rather than a
    /// default so that no caller can assemble a context that teaches one
    /// vocabulary while the gate enforces another.
    pub fn new(schemas: Vec<SchemaPayload>, catalog_hash: String, catalog_prompt: String) -> Self {
        Self {
            schemas,
    ...
    /// (2) the catalog's own prompt, which is the only statement of the output
    /// contract and vocabulary. The goal lives in the user message
    /// (`universal_prompt`).
    ///
    /// Nothing here restates the output format: the catalog's prompt specifies
    /// it, and a second description would be a second contract to drift from.
    pub fn build_system_message(&self) -> String {
        let mut parts = vec![];

        // When generating for a specific plugin, prepend the role-to-component table
        if let Some(target) = &self.target_plugin {
            parts.push("# Plugin Page Generation".to_string());
            parts.push("".to_string());
            parts.push("You are generating a page spec for a single plugin. Use the role-to-component \
                        mapping below to translate schema field roles into catalog components."
                .to_string());
            parts.push("".to_string());
            parts.push("## Role-to-Component Mapping".to_string());
            parts.push(ROLE_COMPONENT_TABLE.to_string());
            parts.push("".to_string());
            parts.push(format!("## Target Plugin: {}", target.plugin_id));
            parts.push("".to_string());

            // Only include the target plugin's schema
            if let Some(schema) = self.schemas.iter().find(|s| s.name == target.plugin_id) {
            ...
            parts.push("".to_string());
            parts.push(
                "Express the dataset using the catalog below, in exactly the output format it \
                 specifies. Stay inside that vocabulary; do not invent component types."
                    .to_string(),
            );
            parts.push("".to_string());
            parts.push("## Dataset (from sealed plugin schemas)".to_string());
            parts.push("Use tools to refine or search; the payloads below are already loaded.".to_string());
            parts.push("".to_string());
        ...
        parts.push("## Catalog (generated — output contract, directives, components)".to_string());
        parts.push(self.catalog_prompt.clone());
        parts.push("".to_string());
...
mod tests {
    ...
    fn system_message_is_dataset_plus_docs_not_ui_roleplay() {
        ...
        let ctx = GenerationContext::new(
            ...
            String::new(),
            "AVAILABLE COMPONENTS (1):\n- statCard: { label: string }".to_string(),
        );
        ...
        // System message: dataset + the generated catalog — not "you are a UI builder".
        assert!(system.contains("# Dataset"));
        assert!(system.contains("### demo"));
        assert!(system.contains("### plain"));
        assert!(system.contains("## Catalog"));
        assert!(
            system.contains("AVAILABLE COMPONENTS"),
            "the catalog's own prompt must reach the model verbatim"
        );
    ...
    #[test]
    fn plugin_scoped_system_message_includes_role_table_and_excludes_other_plugins() {
        use op_state_store::subid_ui::UiSubidProjection;

        // Create two plugins: "target" and "other"
        let target_projection = UiSurfaceProjection {
            subids: vec!["exp.service.target.ui-surfaces@v1".into()],
            routes: vec![UiSurfaceRoute {
                path: "/target".into(),
                name: Some("Target Plugin".into()),
                schema: Some("target".into()),
                raw: serde_json::json!({"path": "/target"}),
            }],
            value_source: Some("default".into()),
        };
        let other_projection = UiSurfaceProjection {
            subids: vec!["exp.service.other.ui-surfaces@v1".into()],
            routes: vec![UiSurfaceRoute {
                path: "/other".into(),
                name: Some("Other Plugin".into()),
                schema: Some("other".into()),
                raw: serde_json::json!({"path": "/other"}),
            }],
            value_source: Some("default".into()),
        };
        ...
        let mut ctx = GenerationContext::new(
            vec![
                payload("target", target_projection.clone()),
                payload("other", other_projection),
            ],
            String::new(),
            "AVAILABLE COMPONENTS".to_string(),
        );

        // Set target_plugin for plugin-scoped generation
        ctx.target_plugin = Some(TargetPluginContext {
            plugin_id: "target".to_string(),
            projections: vec![UiSubidProjection {
                plugin_id: "target".to_string(),
                kind: "field".to_string(),
                id: "status".to_string(),
                subid: "obs.service.target.status@v1".to_string(),
                category: "obs".to_string(),
                role: "display-value".to_string(),
                element_key: "service.target.status@v1".to_string(),
            }],
            ui_surfaces: target_projection,
        });

        let system = ctx.build_system_message();

        // Should contain the role table with UiRole and stateValue
        assert!(
            system.contains("UiRole"),
            "system message should contain UiRole column header from role table"
        );
        assert!(
            system.contains("stateValue"),
            "system message should contain stateValue component from role table"
        );

        // Should contain the target plugin schema
        assert!(
            system.contains("### target"),
            "system message should contain target plugin schema"
        );

        // Should NOT contain the other plugin schema
        assert!(
            !system.contains("### other"),
            "system message should NOT contain other plugin schema when target_plugin is set"
        );

        // Should indicate plugin page generation mode
        assert!(
            system.contains("Plugin Page Generation"),
            "system message should indicate plugin page generation mode"
        );
    }
}
